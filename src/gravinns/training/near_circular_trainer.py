"""Single-orbit angle-domain trainer used by the near-circular study.

Shared numerical machinery lives in the sibling training modules; this file
contains the near-circular optimization protocol.
"""
import json
import os
...
def train_near_circular(nu, p0_factor, r0_km,
                         ...
                         # ── General ──────────────────────────────────────
                         base_dir           = None,
                         run_tag            = None,    # [SWEEP] override leaf folder name
                         auto_resume        = True,    # [SWEEP] False = force fresh run
                         seed               = 0,       # [SWEEP] model-init seed (was hardcoded 0)
                         n_orbits           = 3,
                         n_epochs           = 200_000,
                         n_colloc           = 4000,
                         lr                 = 3e-3,
    ...
    if run_tag is not None:                      # [SWEEP] unique per (IC, seed)
        label = run_tag
    ...
    phi_grid=np.linspace(0,phi_max,4000)
    ...
    L_2pn_ang =torch.tensor(np.full(4000,float(L0)),dtype=torch.float32,device=dev)
    ...
    # ── model ─────────────────────────────────────────────────────────── #
    torch.manual_seed(int(seed))         # [SWEEP] per-run init; was hardcoded 0
    np.random.seed(int(seed) & 0x7fffffff)
    train_near_circular._best_joint=float('inf')  # reset for this run
    model=AngleOrbitPINN4(phi_max,u0,pr0,L0,u_lo,u_hi,dissipative=dissipative,
                          ...
                          pr_max_override=pr_max_eff).to(dev)
    ...
    _pred_shape = _diag.shape_prediction
    ...
    for ep in pbar:
        ...
        if use_causal:
            phi_raw=torch.rand(n_colloc,device=dev)*phi_max
            ...
            phi=(torch.rand(n_colloc,1,device=dev)*phi_max).requires_grad_(True)
            R_u,R_pr,R_L,_,H_col=phi_residual(phi)
        ...
        # ── [SWEEP PATCH] per-epoch loss-component logging ────────────────
        _lrec = _loss_components.LOSS_REC
        if _lrec is not None:   # None == notebook without the LOSS_REC cell
            _lrec.push(epoch=ep, loss=l_val,
                       l_pde=float(l_pde), l_L=float(l_L),
                       l_energy=float(l_energy), l_anchor=float(l_anchor),
                       alpha=float(alpha_cur), beta=float(beta_cur),
        ...
        if ep%log_every==0:
            ...
            if l2s < l2_target:
                _joint=l2s*l2t
                if not hasattr(train_near_circular,'_best_joint'):
                    train_near_circular._best_joint=float('inf')
                if _joint<train_near_circular._best_joint:
                    train_near_circular._best_joint=_joint
            ...
            cur_str=f"α={alpha_cur:.2f}" if alpha_cur>0 else f"S{stage}"
    ...
    _pred_shape_arr = _pred_shape()                       # [SWEEP]
    _ref_shape_arr  = np.stack([qx_r,qy_r,px_r,py_r],1)   # [SWEEP]
    try:
        _ob = _net_out()
        _rec.final.update(best_u=_ob[:,0].astype(np.float32),
                          best_pr=_ob[:,1].astype(np.float32),
                          best_L=_ob[:,2].astype(np.float32),
    ...
    np.savez(os.path.join(ckpt_dir,_ANPZ),t_eval=t_eval,pr0=pr0,ref_2pn_cons=sol_2pn.y.T,
             ...
             pred_best_shape=_pred_shape_arr,ref_shape=_ref_shape_arr,
             dissipative=dissipative,L_ref_ang=L_ref,
             l2_epochs=np.array(l2_ep),l2_shape=np.array(l2s_arr),l2_time=np.array(l2t_arr))
    # ── [SWEEP PATCH] persist the per-epoch loss channels ─────────────
    if _loss_components.LOSS_REC is not None:
        _loss_components.LOSS_REC.save(ckpt_dir)
